...
public class TablaVirtualInteraccion {
	...
	private final String SQLInteraccion3 = "SELECT entregas.id, id_ofrecido, entregas.id_usuario, usuario, usuarios.puntaje, latitud, longitud, entregas.fecha, libros.autor, libros.titulo FROM entregas JOIN ofrecidos ON entregas.id_ofrecido = ofrecidos.id JOIN usuarios ON entregas.id_usuario = usuarios.id JOIN libros ON ofrecidos.id_libro = libros.id WHERE ofrecidos.id_usuario = ? ORDER BY id_ofrecido ASC, entregas.fecha DESC;";
	...
	private final String SQLisEntregado = "SELECT * FROM entregas JOIN ofrecidos ON entregas.id_ofrecido = ofrecidos.id WHERE id_ofrecido = ?;";
	private final String SQLEntregar1 = "INSERT INTO entregas(id_ofrecido, id_usuario) SELECT id_ofrecido, id_usuario FROM reservas WHERE id=?;";
	private final String SQLEntregar2 = "DELETE FROM reservas WHERE id=?";
	private final String SQLEntregar3 = "DELETE FROM solicitudes WHERE id_ofrecido=?";
	...
	public List<Interaccion> getEntregas(Usuario miUsuario) {
		List<Interaccion> entregas = null;
		PreparedStatement pstm = null;
		ResultSet rs = null;
		Connection con = null;
		try {
			con = new ConexionDB(db).getConexion();
			pstm = con.prepareStatement(SQLInteraccion3);
			pstm.setInt(1, miUsuario.getId());
			rs = pstm.executeQuery();
			entregas = new ArrayList<>();
			while(rs.next()){
				int id_entrega = rs.getInt(1);
				int id_ofrecido = rs.getInt(2);
				int id_usuario = rs.getInt(3);
				String usuario = rs.getString(4);
				float puntajeUsuario = rs.getFloat(5);
				...
				String titulo = rs.getString(10);
				String distanciaUsuario = Distancia.haversine(longitud, latitud, miUsuario.getLongitud(), miUsuario.getLatitud());
				Interaccion entrega = new Interaccion(id_entrega,id_ofrecido, id_usuario, usuario, puntajeUsuario, distanciaUsuario, tiempo, autor, titulo);
	...
	public boolean isEntregado(int id_solicitud) {
		boolean exito = false;
		Connection con = new ConexionDB(db).getConexion();
		PreparedStatement pstm= null;
		ResultSet rs = null;
		try {
			pstm = con.prepareStatement(SQLisEntregado);
			pstm.setInt(1, id_solicitud);
			rs = pstm.executeQuery();
			if(rs.next()) {
				exito = true;
	...
	public boolean entregar(int id_reserva, int id_ofrecido) {
		boolean exito = false;
		Connection con = new ConexionDB(db).getConexion();
		PreparedStatement pstm1 = null;
		PreparedStatement pstm2 = null;
		...
		try {
			con.setAutoCommit(false);
			pstm1 = con.prepareStatement(SQLEntregar1);
			pstm1.setInt(1, id_reserva);
			pstm1.execute();
			pstm2 = con.prepareStatement(SQLEntregar2);
			pstm2.setInt(1, id_reserva);
			pstm2.execute();
			pstm3 = con.prepareStatement(SQLEntregar3);
			pstm3.setInt(1, id_ofrecido);
			pstm3.execute();
			con.commit();
			exito = true;
		}
